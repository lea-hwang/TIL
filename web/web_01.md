...
- `input`
  ...
  - input 유형
    ...
    - **동일 항목에 대하여는 `name`을 지정하고 선택한 항목에 대한 `value`를 지정해야 함**
      - `checkbox` : 다중 선택
      - `radio` :  단일 선택
...
- 상속
  ...
  <style>
    p {
      color: red;
      border: 1px solid black;/*상속받지 않음*/
    }
    span {
...
>모든 요소는 네모이고, **위에서부터 아래로,** **왼쪽에서 오른쪽**으로 쌓인다.(**normal flow**)
...
- static: 모든 태그의 기본 값(기준 위치)
  ...
  - 부모 요소 내에서 배체될 때는 부모 요소의 위치를 기준으로 배치됨
...
- absolute: 절대 위치
  ...
  - **static이 아닌 가장 가까이 있는 부모/조상 요소**를 기준으로 이동(없는 경우 body)
